/*
Name        :   Ashwin Raj K
Description :   Steganography - Decoding
Date        :   06/06/2023
*/
...
{
    if (open_files_dec(decInfo) == d_success)               //checks if files can be opened
    {
        printf("Open files is a success\n");
        if (decode_magic_string(decInfo) == d_success)                      //checks if magic string is decoded
        {
            printf("Decoded magic string Successfully\n");
            if (decode_file_extn_size(strlen(".txt"), decInfo->fptr_d_src_image) == d_success)      //checks if file ext size is decoded
            {
                printf("Decoded file extension size Succesfully\n");
                if (decode_secret_file_extn(decInfo->d_extn_secret_file, decInfo) == d_success)     //checks if secret file ext decoded
                {
                    printf("Decoded Secret File Extension Succesfully\n");
                    if (decode_secret_file_size(decInfo->size_secret_file, decInfo) == d_success)    //checks if secret file size decoded
                    {
                        printf("Decoded secret file size Successfully\n");
                        if (decode_secret_file_data(decInfo) == d_success)                  //checks if secret file data decoded
                        {
                    ...
                    {
                        printf("Decode of secret file size is a failure\n");     //prints Decode of secret file size is a failure and returns d_failure
                        return d_failure;
                    }
                }
                else
                {
                    printf("Decode of Secret file extension is a failure\n");      //prints Decode of Secret file extension is a failure and returns d_failure
                    return d_failure;
                }
            }
            else
            {
                printf("Decoded of file extension size is a failure\n");        //prints Decoded of file extension size is a failure and returns d_failure
                return d_failure;
            }
        }
        else
        {
            printf("Decoding of magic string is a failure\n");              //prints Decoding of magic string is a failure and returns d_failure
            return d_failure;
        }
    }
    else
    {
        printf("Open files is a failure\n");                    //prints Open files is a failure and returns d_failure
        return d_failure;
    }
    return d_success;                                           //returns d_success
}
